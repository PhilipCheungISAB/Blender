    ...
    for j in range(num_layers):
        ...
        if j % 2 == 0:
            for i in range(N):
                ...
                # CRITICAL: 10.0cm initial bow (positive Y for front wall to bulge INWARD)
                bow_y = 0.10 * math.cos(dist_x / 2.3 * math.pi/2) * math.cos(dist_z / 1.25 * math.pi/2)
    ...
    for j in range(num_layers):
        ...
        if j % 2 == 0:
            for i in range(N):
                ...
                # CRITICAL: 10.0cm initial bow (negative X for right wall to bulge INWARD)
                bow_x = -0.10 * math.cos(dist_y / 2.3 * math.pi/2) * math.cos(dist_z / 1.25 * math.pi/2)
...
def main():
    ...
    filepath = "e:\\Antigravity\\Building\\anti_seismic_sim_large_plastic.blend"
