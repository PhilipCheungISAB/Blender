...
def create_middle_anchor_constraint(brick, anchor, location, collection, limit_z_lower=-0.20):
    empty = bpy.data.objects.new(f"Con_Mid_{brick.name}", None)
    collection.objects.link(empty)
    empty.location = location
    ...
    # Allow Z translation (vertical slide/sag) up to limit_z_lower
    con.use_limit_lin_z = True
    con.limit_lin_z_lower = limit_z_lower
    con.limit_lin_z_upper = 0.0
    
    # Release all rotations
    con.use_limit_ang_x = False
    con.use_limit_ang_y = False
    con.use_limit_ang_z = False
...
def add_constraints_wall(wall_bricks, fixed_axis, fixed_coord, frame, foundation, is_x_wall=True):
    ...
    # Anchor to frame (top beam) - GENERIC sliding constraint (limit 20cm slide)
    for idx_b, (b, t, pos) in enumerate(wall_bricks[-1]):
        ...
        create_generic_constraint(b, anchor_obj, loc, constraints_group, unbreakable=True, limit_z_lower=-0.20)
        boundary_anchors.append((anchor_obj, loc[0], loc[1], loc[2]))
        
    # Anchor to frame (side columns) - GENERIC sliding constraint (limit 20cm slide)
    for j, layer in enumerate(wall_bricks):
        z = j * (H + g) + H/2 + g
        ...
        create_generic_constraint(b_l, anchor_l, loc_l, constraints_group, unbreakable=True, limit_z_lower=-0.20)
        boundary_anchors.append((anchor_l, loc_l[0], loc_l[1], loc_l[2]))
        ...
        create_generic_constraint(b_r, anchor_r, loc_r, constraints_group, unbreakable=True, limit_z_lower=-0.20)
        boundary_anchors.append((anchor_r, loc_r[0], loc_r[1], loc_r[2]))
        
    # Middle Anchors for layers 1 to 22
    for j in range(1, len(wall_bricks) - 1):
        layer = wall_bricks[j]
        middle_brick = None
        min_dist = 9999.0
        ...
        if middle_brick:
            z = j * (H + g) + H/2 + g
            loc = (middle_brick.location.x, middle_brick.location.y, z)
            anchor_mid = create_physics_anchor(f"Anchor_Mid_{middle_brick.name}", loc, constraints_group)
            create_middle_anchor_constraint(middle_brick, anchor_mid, loc, constraints_group, limit_z_lower=-0.20)
            middle_anchors.append((anchor_mid, loc[0], loc[1], loc[2]))
    ...
    return boundary_anchors, middle_anchors
...
def main():
    ...
    front_bounds, front_mids = add_constraints_wall(wall_front, 'X', -2.4, frame, foundation, is_x_wall=True)
    right_bounds, right_mids = add_constraints_wall(wall_right, 'Y', 2.4, frame, foundation, is_x_wall=False)
    ...
    for f in range(1, 251):
        ...
        # Calculate bowing amplitude for this frame: 0.0 at frame 1 to 20cm (0.20m) at frame 180
        if f <= 1:
            bow_amp = 0.0
        elif f >= 180:
            bow_amp = 0.20
            ...
            p = (f - 1) / (180 - 1)
            bow_amp = 0.20 * (3 * p**2 - 2 * p**3)
        
        # Keyframe boundary anchors (position & rotation based on Hermite S-Curve, sway only)
        for anchor_obj, loc_x, loc_y, loc_z in front_bounds + right_bounds:
            u = loc_z / 2.7
        ...
        # Keyframe front wall middle anchors (sway + concave bow in +Y direction)
        for anchor_obj, loc_x, loc_y, loc_z in front_mids:
            u = loc_z / 2.7
            factor = (3 * u**2 - 2 * u**3) if loc_z > 0 else 0.0
            dx = vx * 0.5 * factor
            dy = vy * 0.5 * factor
            
            # Add out-of-plane concave bow in Y-direction
            u_z = max(0.0, min(2.5, loc_z))
            vertical_factor = math.sin((u_z / 2.5) * math.pi)
            dy_bow = bow_amp * vertical_factor
            
            anchor_obj.location.x = loc_x + dx
            anchor_obj.location.y = loc_y + dy + dy_bow
            anchor_obj.location.z = loc_z
        ...
        # Keyframe right wall middle anchors (sway + convex bow in +X direction)
        for anchor_obj, loc_x, loc_y, loc_z in right_mids:
            u = loc_z / 2.7
            factor = (3 * u**2 - 2 * u**3) if loc_z > 0 else 0.0
            dx = vx * 0.5 * factor
            dy = vy * 0.5 * factor
            
            # Add out-of-plane convex bow in X-direction
            u_z = max(0.0, min(2.5, loc_z))
            vertical_factor = math.sin((u_z / 2.5) * math.pi)
            dx_bow = bow_amp * vertical_factor
            
            anchor_obj.location.x = loc_x + dx + dx_bow
            anchor_obj.location.y = loc_y + dy
            anchor_obj.location.z = loc_z
    ...
    filepath = "e:\\Antigravity\\Building\\anti_seismic_sim_large_plastic_v2.blend"
