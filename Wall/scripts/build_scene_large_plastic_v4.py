...
def create_brick_generic_constraint(obj1, obj2, location, collection, limit_xyz=0.015, limit_ang=math.radians(2.5)):
    empty = bpy.data.objects.new(f"Con_{obj1.name}_{obj2.name}", None)
    collection.objects.link(empty)
    empty.location = location
    ...
    # Disable Z translation limit to prevent vertical fight with anchor sag
    con.use_limit_lin_z = False
...
def create_brick_anchor_constraint(brick, anchor, location, collection):
    empty = bpy.data.objects.new(f"Con_Anchor_{brick.name}", None)
    collection.objects.link(empty)
    empty.location = location
    ...
    con = empty.rigid_body_constraint
    con.type = 'GENERIC'
    con.object1 = anchor   # Anchor is object1 (kinematic)
    con.object2 = brick    # Brick is object2 (dynamic)
    con.disable_collisions = False
    con.use_breaking = False  # Unbreakable
    
    # Lock X and Y translations relative to the anchor
    con.use_limit_lin_x = True
    con.limit_lin_x_lower = 0.0
    con.limit_lin_x_upper = 0.0
    ...
    # Lock Z translation relative to the anchor to force the brick to follow the anchor's sag
    con.use_limit_lin_z = True
    con.limit_lin_z_lower = 0.0
    con.limit_lin_z_upper = 0.0
    
    # Lock all rotations relative to the anchor to force the brick to follow anchor rotation
    con.use_limit_ang_x = True
    con.limit_ang_x_lower = 0.0
    con.limit_ang_x_upper = 0.0
...
def get_skew_factor(pos, pos_0, boundary=2.3):
    if pos <= pos_0:
        w = pos_0 - (-boundary)
        if w == 0: return 0.0
        return math.cos((pos - pos_0) / w * math.pi/2)
    else:
        w = boundary - pos_0
        if w == 0: return 0.0
        return math.cos((pos - pos_0) / w * math.pi/2)
...
def add_constraints_wall(wall_bricks, fixed_axis, fixed_coord, frame, foundation, is_x_wall=True):
    ...
    # Create anchor for EVERY brick in layers 0 to 23 (including bottom row to allow sag sinking)
    for j in range(len(wall_bricks)):
        layer = wall_bricks[j]
        for b, t, pos in layer:
            z = j * (H + g) + H/2 + g
            loc = (b.location.x, b.location.y, z)
            
            anchor_obj = create_physics_anchor(f"Anchor_{b.name}", loc, constraints_group)
            create_brick_anchor_constraint(b, anchor_obj, loc, constraints_group)
            brick_anchors.append((anchor_obj, loc[0], loc[1], loc[2]))
...
def main():
    ...
    # Set foundation to Group 2 to avoid collision with bricks (Group 1) when bottom row sags/sinks
    foundation.rigid_body.collision_collections[0] = False
    foundation.rigid_body.collision_collections[1] = True
    
    # 2. Unified Frame with Shape Key Swaying
    frame = create_unified_frame("Unified_Frame", concrete_mat)
    
    # 3. Brick Walls (Front and Right are initially perfectly flat)
    ...
    for f in range(1, 251):
        ...
        # Calculate bowing amplitude and sag amplitude for this frame
        # Bowing amplitude: 0.0 at frame 1 to 10cm (0.10m) at frame 180
        # Sag amplitude: 0.0 at frame 1 to 0.5 brick heights (0.05m) at frame 180
        if f <= 1:
            bow_amp = 0.0
            sag_amp = 0.0
        elif f >= 180:
        ...
        # Keyframe front wall anchors (sway + concave bow in +Y + bowing rotation + Z sag)
        for anchor_obj, loc_x, loc_y, loc_z in front_anchors:
            u = loc_z / 2.7
            factor = (3 * u**2 - 2 * u**3) if loc_z > 0 else 0.0
            dx_sway = vx * 0.5 * factor
            dy_sway = vy * 0.5 * factor
            
            # Bowing displacement (Shifted peak at pos_0 = 1.15)
            u_pos = max(-2.3, min(2.3, loc_x))
            u_z = max(0.0, min(2.5, loc_z))
            
            pos_0 = 1.15
            if u_pos <= pos_0:
                w = pos_0 + 2.3
                cos_x = math.cos((u_pos - pos_0) / w * math.pi/2)
                sin_x = math.sin((u_pos - pos_0) / w * math.pi/2)
                slope_x_factor = - (math.pi / (2 * w)) * sin_x
            else:
                w = 2.3 - pos_0
                cos_x = math.cos((u_pos - pos_0) / w * math.pi/2)
                sin_x = math.sin((u_pos - pos_0) / w * math.pi/2)
            ...
            sin_z = math.sin(u_z / 2.5 * math.pi)
            dy_bow = bow_amp * cos_x * sin_z
            
            # Sag displacement (using same skewed cosine factor cos_x)
            dz_sag = -sag_amp * cos_x
            
            anchor_obj.location.x = loc_x + dx_sway
            anchor_obj.location.y = loc_y + dy_sway + dy_bow
            anchor_obj.location.z = loc_z + dz_sag
            
            # Sway rotation
            slope_factor = (3.0 / 2.7 * (u - u**2)) if loc_z > 0 else 0.0
            rot_y_sway = vx * 0.5 * slope_factor
            rot_x_sway = -vy * 0.5 * slope_factor
            rot_z_sway = 0.0
            
            # Bowing rotation
            slope_x = bow_amp * slope_x_factor * sin_z
            cos_z = math.cos(u_z / 2.5 * math.pi)
            slope_z = bow_amp * cos_x * (math.pi / 2.5) * cos_z
            
            rot_x_bow = -math.atan(slope_z)
            rot_y_bow = 0.0
            rot_z_bow = math.atan(slope_x)
            
            anchor_obj.rotation_euler = (rot_x_sway + rot_x_bow, rot_y_sway + rot_y_bow, rot_z_sway + rot_z_bow)
            
            anchor_obj.keyframe_insert("location", frame=f)
            anchor_obj.keyframe_insert("rotation_euler", frame=f)
            
        # Keyframe right wall anchors (sway + convex bow in +X + bowing rotation + Z sag)
        for anchor_obj, loc_x, loc_y, loc_z in right_anchors:
            u = loc_z / 2.7
            factor = (3 * u**2 - 2 * u**3) if loc_z > 0 else 0.0
            dx_sway = vx * 0.5 * factor
            dy_sway = vy * 0.5 * factor
            ...
            # Bowing displacement (Shifted peak at pos_0 = -1.15)
            u_pos = max(-2.3, min(2.3, loc_y))
            u_z = max(0.0, min(2.5, loc_z))
            
            pos_0 = -1.15
            if u_pos <= pos_0:
                w = pos_0 + 2.3
                cos_y = math.cos((u_pos - pos_0) / w * math.pi/2)
                sin_y = math.sin((u_pos - pos_0) / w * math.pi/2)
                slope_y_factor = - (math.pi / (2 * w)) * sin_y
            else:
                w = 2.3 - pos_0
                cos_y = math.cos((u_pos - pos_0) / w * math.pi/2)
                sin_y = math.sin((u_pos - pos_0) / w * math.pi/2)
            ...
            sin_z = math.sin(u_z / 2.5 * math.pi)
            dx_bow = bow_amp * cos_y * sin_z
            
            # Sag displacement (using same skewed cosine factor cos_y)
            dz_sag = -sag_amp * cos_y
            
            anchor_obj.location.x = loc_x + dx_sway + dx_bow
            anchor_obj.location.y = loc_y + dy_sway
            anchor_obj.location.z = loc_z + dz_sag
            ...
            # Bowing rotation
            slope_y = bow_amp * slope_y_factor * sin_z
            cos_z = math.cos(u_z / 2.5 * math.pi)
            slope_z = bow_amp * cos_y * (math.pi / 2.5) * cos_z
            
            rot_x_bow = 0.0
            rot_y_bow = math.atan(slope_z)
            rot_z_bow = -math.atan(slope_y)
    ...
    filepath = "e:\\Antigravity\\Building\\anti_seismic_sim_large_plastic_v4.blend"
