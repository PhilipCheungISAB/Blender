...
def create_brick_anchor_constraint(brick, anchor, location, collection, limit_z_lower):
...
def add_constraints_wall(wall_bricks, fixed_axis, fixed_coord, frame, foundation, is_x_wall=True):
    ...
    # Create anchor for EVERY brick in layers 1 to 23
    for j in range(1, len(wall_bricks)):
        layer = wall_bricks[j]
        for b, t, pos in layer:
            z = j * (H + g) + H/2 + g
            loc = (b.location.x, b.location.y, z)
            
            # Determine Z-slide limit based on horizontal position (0.5 brick height center, 1cm edge)
            u_pos = max(-2.3, min(2.3, pos))
            limit_z = -0.01 - 0.041 * math.cos(u_pos / 2.3 * math.pi/2)
            
            anchor_obj = create_physics_anchor(f"Anchor_{b.name}", loc, constraints_group)
            create_brick_anchor_constraint(b, anchor_obj, loc, constraints_group, limit_z_lower=limit_z)
...
def main():
    ...
    for f in range(1, 251):
        ...
        # Keyframe front wall anchors (sway + concave bow in +Y + bowing rotation)
        for anchor_obj, loc_x, loc_y, loc_z in front_anchors:
            ...
            cos_x = math.cos(u_pos / 2.3 * math.pi/2)
            ...
            # Bowing rotation
            sin_x = math.sin(u_pos / 2.3 * math.pi/2)
            slope_x = -bow_amp * (math.pi / 4.6) * sin_x * sin_z
        ...
        # Keyframe right wall anchors (sway + convex bow in +X + bowing rotation)
        for anchor_obj, loc_x, loc_y, loc_z in right_anchors:
            ...
            cos_y = math.cos(u_pos / 2.3 * math.pi/2)
            sin_z = math.sin(u_z / 2.5 * math.pi)
            ...
            # Bowing rotation
            sin_y = math.sin(u_pos / 2.3 * math.pi/2)
            slope_y = -bow_amp * (math.pi / 4.6) * sin_y * sin_z
    ...
    filepath = "e:\\Antigravity\\Building\\anti_seismic_sim_large_plastic_v3.blend"
